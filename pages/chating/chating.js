import { generateFingerGuessImageFile, generateBigEmojiImageFile, generateRichTextNode, generateImageNode, calcTimeHeader } from '../../imutils/util.js'
import { deepClone, clickLogoJumpToCard } from '../../imutils/util.js'
import * as iconBase64Map from '../../imutils/imageBase64.js'
...
Page({
  ...
  onLoad: function (options) {
    ...
    // 渲染应用期间历史消息
    let loginUserAccount = app.globalData['loginUser']['account']//我的账户
    let loginMessageList = app.globalData.messageList[loginUserAccount]//被聊天账号列表
    if (Object.keys(loginMessageList).length != 0) {
      let chatToMessageList = loginMessageList[chatTo]//被聊天对象
      ...
      tempArr.push({
        type:'custom',
        goodsSrc:'https://dev.yunxin.163.com/images/logo2.png',
        goodsName:'中秋月饼',
        goodsPrice:'￥100',
        goodsId:'222',
        time:nowTime
      ...
      if (newMessage.type === 'text') {
        ...
        this.setData({
          messageArr: [...this.data.messageArr, {
            ...
            sendOrReceive: newMessage.sendOrReceive,
            displayTimeHeader,
            goodsPrice: newMessage.price,
            goodsSrc: newMessage.cover,
            goodsName: newMessage.title,
            goodsId: newMessage.gid
          }]
        })
 ...
 */
  ...
  sendGoddsMes(){
    ...
    var content = {
      type: 1,
      data:{
        cover: 'https://dev.yunxin.163.com/images/logo2.png',
        title: '中秋月饼',
        price: '￥100',
        gid: '222'
      }
    };
    var msg = app.globalData.nim.sendCustomMsg({
      scene: 'p2p',
      to: this.data.chatTo,
      content: JSON.stringify(content),
      done: function(error, msg) {
        if (self.handleErrorAfterSend(error)) {
          return
        }
        console.log(msg)
        // 刷新界面
        ...
        self.setData({
          messageArr: [...self.data.messageArr, {
            ...
            sendOrReceive: 'send',
            displayTimeHeader,
            goodsPrice: content.price,
            goodsSrc: content.cover,
            goodsName:content.title,
            goodsId: content.gid
          }]
        })
        ...
        // 最后一个参数表示，不更新未读数
        app.globalData.subscriber.emit('UPDATE_RECENT_CHAT', { account: msg.to, time: msg.time, content: msg.content, type: msg.type }, true)
        // 滚动到底部
        self.scrollToBottom()
      }
    });
