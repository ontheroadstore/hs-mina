...
Page({
  ...
  data: {
    ...
    spescPopupStatus: false,        //是否显示规格
    problemList: null,              //常见问题
    tabContentProblem: 1,           //商品详情和购物须知tab切换 1： 商品详情 2：常见问题
    limitBuyNum: 0                  //限购数量
  },
  onLoad: function (options) {
    ...
    req(app.globalData.bastUrl, 'appv6_5/getCommodityDetail/' + this.data.articleId+'?mask=1').then(res => {
      ...
      this.setData({
        // fullReduceInfo: res.data.sale_promotion,//未知
        modulesGuessLike: res.data.related_goods,
        desc: util.replaceBr(res.data.post.desc),
        content: util.replaceBr(res.data.post.content)
      })
      ...
      // 神策 浏览商品详情页
      app.sensors.track('commodityDetail', { commodityID: String(this.data.articleId), sellerID: String(res.data.seller_related_goods.id) });
    ...
    //获取用户须知
    req(app.globalData.bastUrl, `appv6_1/goods/${this.data.articleId}/assessment`).then(res => {
      if (res.data) {
        this.setData({
          problemList: res.data.problem
        })
      }
    })
  },
  ...
  changeTabStatus(e){
    console.log(e)
    this.setData({
      tabContentProblem: e.currentTarget.dataset.type
    })
  },
  //显示规格弹框
  showSpecsPopup(){
    this.setData({
      spescPopupStatus: true
    })
  },
  hideSpecsPopup(){
    this.setData({
      spescPopupStatus: false
    })
  },
  // 获取领券和返券
